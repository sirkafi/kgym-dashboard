...
import { ArrowLeft, Save, MapPin, Key, Shield, Trash2 } from 'lucide-react';
    ...
    <div>
      <div className="page-header">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1>Edit Branch Configuration</h1>
            <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem', marginTop: '0.25rem' }}>Update settings and admin access for Branch ID: {id || '1'}</p>
          </div>
        </div>
        <button className="btn btn-outline" style={{ color: 'var(--danger)', borderColor: 'var(--danger)' }}>
          <Trash2 size={16} /> Disable Branch
        </button>
      </div>
      ...
      <div className="grid" style={{ gridTemplateColumns: '1fr 1fr', gap: 'var(--spacing-6)' }}>
        ...
        <div className="flex-col gap-6" style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-6)' }}>
          <div className="card">
            ...
            <div className="form-group">
              ...
              <input type="text" className="form-input" defaultValue="KGym Pusat" />
            </div>

            <div className="form-group">
              <label className="form-label">Full Address</label>
              <textarea className="form-input" rows={3} defaultValue="Jl. Jendral Sudirman No. 123, Central Business District"></textarea>
            </div>
          ...
          <div className="card">
            ...
            <div className="grid" style={{ gridTemplateColumns: '1fr 1fr', gap: 'var(--spacing-3)' }}>
              {['Cardio Area', 'Free Weights', 'Machines', 'Locker Room', 'Shower', 'Sauna', 'Studio Class', 'Cafe / Lounge'].map((fac, idx) => (
                <label key={fac} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.875rem', cursor: 'pointer' }}>
                  <input type="checkbox" defaultChecked={idx < 5} style={{ width: '16px', height: '16px', accentColor: 'var(--primary-red)' }} />
                  {fac}
                </label>
              ))}
            </div>
        ...
        <div className="flex-col gap-6" style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-6)' }}>
          <div className="card">
            <h3 style={{ marginBottom: 'var(--spacing-4)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <Key size={18} /> Update Branch Admin
            </h3>
            ...
            <div className="form-group">
              ...
              <input type="email" className="form-input" defaultValue="budi.pusat@kgym.com" />
            </div>
